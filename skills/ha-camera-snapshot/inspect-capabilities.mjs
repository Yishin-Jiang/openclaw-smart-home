...
const result = await new Promise((resolve, reject) => {
  ...
  const timer = setTimeout(() => reject(new Error('Capability request timed out')), 10_000)
  ...
  socket.addEventListener('message', (event) => {
    ...
    else if (message.type === 'auth_ok') socket.send(JSON.stringify({ id: 1, type: 'camera/capabilities', entity_id: entityId }))
    else if (message.type === 'result' && message.id === 1) finish(message.success ? null : new Error(message.error?.message || 'Capability request failed'), message.result)
  })
  socket.addEventListener('error', () => finish(new Error('Home Assistant WebSocket failed')))
})

console.log(JSON.stringify({ entityId, capabilities: result }, null, 2))
