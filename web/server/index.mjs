  ...
  try {
    const upstream = await fetch(`${gatewayUrl}/v1/chat/completions`, {
      ...
      body: JSON.stringify({
        ...
        stream: true,
        messages: [
          { role: 'system', content: `${executionProtocol} ${cameraPolicy.captureRequested
            ? `The user explicitly authorized one fresh camera capture for this request. Use the ha-camera-snapshot skill. ${cameraPolicy.imageRequested ? 'They also explicitly requested to receive the image; retain it through response delivery and include it as instructed by the skill.' : 'They requested visual analysis only, not delivery of the image.'}`
    ...
    if (observed.actionFailed) {
      ...
      assistantText = '設備操作已送出，但 OpenClaw 沒有完成事後狀態驗證，因此目前不能確認操作已成功。'
    }
...
const server = createServer(async (request, response) => {
  try {
    ...
    if (url.pathname.startsWith('/api/')) {
      ...
      if (request.method === 'POST' && (url.pathname === '/api/chat' || url.pathname === '/api/energy/insights')) {
